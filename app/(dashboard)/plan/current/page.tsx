...
import { ThemeInput } from "@/components/plan/ThemeInput";
...
export default function CurrentPlanPage() {
  ...
  // ---- F4 theme + locks ----
  // Per-video lock state — survives "Regenerate all" so users can pin
  // the cards they love. Purely client-side; no API surface.
  ...
  const generate = useCallback(
    async (opts: {
      ...
      /** F4 — natural-language theme/hint for autopilot mode (≤ 140 chars). */
      autopilotHint?: string;
      /** F4 — preserve videos whose ids are in the lockedIds set. */
      ...
      try {
        const res = await fetch("/api/plan/generate", {
          ...
          body: JSON.stringify({
            ...
            autopilotHint: opts.autopilotHint,
          }),
        });
        ...
        const fresh = json.data.videos as PlanVideo[];
        if (opts.preserveLocks && videos) {
          // Merge: keep locked videos in place at their day slot; fill the
          // rest from the fresh response in day order.
          const keptByDay = new Map<string, PlanVideo>();
          for (const v of videos) {
            if (lockedIds.has(v.id)) {
              keptByDay.set(v.dayOfWeek.toLowerCase(), v);
            }
          }
          const freshQueue = [...fresh];
          const merged: PlanVideo[] = [];
          const dayOrder = ["mon", "tue", "wed", "thu", "fri", "sat", "sun"];
          for (const day of dayOrder) {
            const locked = keptByDay.get(day);
            if (locked) {
              merged.push(locked);
            } else {
              const nextFresh = freshQueue.shift();
              if (nextFresh) merged.push({ ...nextFresh, dayOfWeek: day });
            }
          }
          setVideos(merged);
          toast.success(
            `Regenerated ${merged.length - keptByDay.size} card${
              merged.length - keptByDay.size === 1 ? "" : "s"
            }. ${keptByDay.size} locked.`,
          );
        } else {
          setVideos(fresh);
        }
        ...
        if (opts.autopilotHint !== undefined) setLastTheme(opts.autopilotHint);
      } catch {
    ...
    [platforms, niche, tone, contentLanguage, videos, lockedIds],
  );

  // F4 — toggle lock for a video id.
    ...
    return (
      ...
      >
        <FirstRunHint
          ...
          message="Click any day to review. Lock cards to pin them through a regenerate. Destructive actions get an Undo."
          className="mb-3"
        />
        ...
        {mode === "autopilot" ? (
          <div className="mb-3">
            <ThemeInput
              defaultValue={lastTheme}
              placeholder="Theme for this week — or leave blank for AI picks."
              buttonLabel={lockedIds.size > 0 ? "Regenerate unlocked" : "Regenerate all"}
              loading={loading}
              onSubmit={(theme) =>
                generate({
                  mode: "autopilot",
                  autopilotHint: theme || undefined,
                  preserveLocks: true,
                })
              }
            />
            {lockedIds.size > 0 ? (
              <p className="mt-1.5 text-[11px] text-text-tertiary">
                <span className="tabular-nums">{lockedIds.size}</span> card
                {lockedIds.size === 1 ? "" : "s"} locked — they'll survive the
                next regenerate.
              </p>
            ) : null}
          </div>
            ...
            {videos.map((v, i) => (
              <li key={v.id}>
                <DayCard
                  ...
                  onRegenerate={() =>
                    toast(
                      "Per-card regenerate lands next. Lock the cards you love and hit 'Regenerate all'.",
                    )
                  }
                  onPublish={(id) => {
                    setDrawerId(id);

...
import { useOnWindowResize } from "@/lib/hooks/use-on-window-resize"
import { cx, percentageFormatter } from "@/lib/utils"
import { Badge } from "./Badge"
import { getBadgeType } from "@/components/dashboard/overview/DashboardChartCard"
...
const LegendItem = ({
  ...
  return (
    <li
      className={cx(
        // base
        "group inline-flex flex-nowrap items-center gap-1.5 whitespace-nowrap rounded px-3 py-2.5 transition",
        hasOnValueChange
...
const ChartLegend = (
  { payload }: { payload?: readonly LegendPayloadEntry[] },
  ...
  const filteredPayload = (payload ?? [])
    .filter((item): item is LegendPayloadEntry & { value: string } =>
    ...
    <div ref={legendRef} className="flex items-center justify-end">
      <Legend
        ...
        colors={filteredPayload
          .map((entry) => categoryColors.get(entry.value))
          .filter(
          ...
          >
            ...
            <XAxis
              ...
              ticks={
                startEndOnly
                  ? [
                      data[0][index] as string | number,
                      data[data.length - 1][index] as string | number,
                    ]
                  : undefined
              }
              fill=""
              ...
              <Line
                ...
                activeDot={(props) => {
                  const {
                    ...
                    strokeLinejoin?: React.SVGProps<SVGElement>["strokeLinejoin"]
                    strokeWidth?: number
                    dataKey: string
                    index: number
                    payload: Record<string, unknown>
            ...
            {onValueChange
              ? categories.map((category) => (
                  <Line
                    ...
                    strokeWidth={12}
                    connectNulls={connectNulls}
                    onClick={(props: { name?: string }, event) => {
                      event.stopPropagation()
                      const { name } = props
